/*
 * Measure_Pw.ino — Flash計測用 BASE + Pw (PWM_DUTY)
 */
#include <Arduino.h>
#include <WebHID.h>
#include <SDmin.h>
...
static void stream_bytes(const uint8_t *data,uint8_t len){
  uint8_t offset=0;
  while(offset<len){uint8_t chunk=len-offset;if(chunk>5)chunk=5;rsp(RSP_DATA,data+offset,chunk);offset+=chunk;}
}
...
static void hidLog(uint8_t type,uint8_t d0=0,uint8_t d1=0,uint8_t d2=0,uint8_t d3=0,uint8_t d4=0,uint8_t d5=0){
  uint8_t p[8]={'D',type,d0,d1,d2,d3,d4,d5};WebHID.send(p,8);
}
...
static void consoleWriteChunk(const char *s,uint8_t len,bool more){
  ...
  uint8_t n=len>6?6:len;for(uint8_t i=0;i<n;i++)buf[2+i]=(uint8_t)s[i];
  WebHID.send(buf,8);delay(12);
}
static void consolePrint(const char *s,uint8_t len,uint8_t flags){
  ...
  if(inspect)tmp[tlen++]='"';if(newline)tmp[tlen++]='\n';
  uint8_t pos=0;
  while(pos<tlen){uint8_t chunk=tlen-pos;if(chunk>6)chunk=6;bool more=(pos+chunk<tlen);consoleWriteChunk(tmp+pos,chunk,more);pos+=chunk;}
}
...
static void handleOpenW(){
  ...
  if(!fname[0]){rsp_err();return;}if(!sm_open_w(fname)){rsp_err();return;}
...
static void handleListDir(){
  char dir[27];get_name(cmdBuf+1,dir);SmListCtx ctx;
  if(!sm_list_open(&ctx,dir[0]?dir:NULL)){rsp_err();return;}
  char fname[27];uint8_t entry[29];int typ;
  while((typ=sm_list_next(&ctx,fname))>0){uint8_t nlen=(uint8_t)strlen(fname);entry[0]=(typ==2)?'D':'F';memcpy(entry+1,fname,nlen);entry[1+nlen]=0;stream_bytes(entry,2+nlen);}
  rsp_end();
}
#define OP_END           0x00
...
#define OP_PWM_BASE_FREQ 0x24  // uint8 pin, uint16 freq → PSC = 48MHz/(256*freq)-1
// ────────
...
static bool seekTo(const char *filename,uint16_t target_pc){
  sm_close_r();if(!sm_open_r(filename))return false;
  uint8_t hdr[8];if(sm_read_full(hdr,8)!=8)return false;
...
static bool runUap(const char *filename){
  hidLog(LOG_UAP_START);if(!sm_open_r(filename))return false;
  uint8_t header[8];if(sm_read_full(header,8)!=8){sm_close_r();return false;}
  if(header[0]!='U'||header[1]!='R'||header[2]!='B'||header[3]!='1'){hidLog(LOG_MAGIC_FAIL,header[0],header[1],header[2],header[3]);sm_close_r();return false;}
  ...
  while(pc<codeSize){
    ...
    switch(opcode){
      ...
      case OP_GPIO_MODE:{uint8_t b[2];if(sm_read_full(b,2)!=2)goto vm_err;pc+=2;
      ...
      case OP_PWM_BASE_FREQ:{
        ...
        uint16_t freq=(uint16_t)b[1]|((uint16_t)b[2]<<8);if(!freq)freq=1;
        uint16_t psc=(uint16_t)(48000000UL/(256UL*freq)-1);
        if(b[0]==2){_pwm_psc[0]=psc;}else{_pwm_psc[1]=psc;}
        break;
      }
      case OP_PWM_DUTY:{
        ...
        if(b[0]==2){
          ...
          if(b[0]==5){gpio=GPIOC;rcc_gpio=RCC_IOPCEN;cfglr_mask=0xFU<<12;cfglr_val=0xBU<<12;cvr=&TIM1->CH3CVR;chctlr=&TIM1->CHCTLR2;ctlr_mask=0x0070U;ctlr_val=0x0060U;ccer_bit=TIM_CC3E;}
          else if(b[0]==0){gpio=GPIOA;rcc_gpio=RCC_IOPAEN;cfglr_mask=0xFU<<4;cfglr_val=0xBU<<4;cvr=&TIM1->CH2CVR;chctlr=&TIM1->CHCTLR1;ctlr_mask=0x7000U;ctlr_val=0x6000U;ccer_bit=TIM_CC2E;}
          else{gpio=GPIOD;rcc_gpio=RCC_IOPDEN;cfglr_mask=0xFU<<8;cfglr_val=0xBU<<8;cvr=&TIM1->CH1CVR;chctlr=&TIM1->CHCTLR1;ctlr_mask=0x0070U;ctlr_val=0x0060U;ccer_bit=TIM_CC1E;}
      ...
      // ────────────────────────────────────────────────────────────
      case OP_JMP:{uint8_t b[2];if(sm_read_full(b,2)!=2)goto vm_err;pc+=2;int16_t offset=(int16_t)((uint16_t)b[0]|((uint16_t)b[1]<<8));int32_t new_pc=(int32_t)pc+(int32_t)offset;if(new_pc<0||new_pc>(int32_t)codeSize)goto vm_err;if(!seekTo(filename,(uint16_t)new_pc))goto vm_err;pc=(uint16_t)new_pc;break;}
      case OP_JZ:{uint8_t b[3];if(sm_read_full(b,3)!=3)goto vm_err;pc+=3;if(regs[b[0]&3]==0){int16_t offset=(int16_t)((uint16_t)b[1]|((uint16_t)b[2]<<8));int32_t new_pc=(int32_t)pc+(int32_t)offset;if(new_pc<0||new_pc>(int32_t)codeSize)goto vm_err;if(!seekTo(filename,(uint16_t)new_pc))goto vm_err;pc=(uint16_t)new_pc;}break;}
      case OP_JNZ:{uint8_t b[3];if(sm_read_full(b,3)!=3)goto vm_err;pc+=3;if(regs[b[0]&3]!=0){int16_t offset=(int16_t)((uint16_t)b[1]|((uint16_t)b[2]<<8));int32_t new_pc=(int32_t)pc+(int32_t)offset;if(new_pc<0||new_pc>(int32_t)codeSize)goto vm_err;if(!seekTo(filename,(uint16_t)new_pc))goto vm_err;pc=(uint16_t)new_pc;}break;}
...
void loop(){
  if(autoRun){autoRun=false;digitalWrite(LED_PIN,HIGH);runUap("main.urb");digitalWrite(LED_PIN,LOW);delay(200);}
  uint8_t cmd=recvCmd();
  switch(cmd){case CMD_RUN:digitalWrite(LED_PIN,HIGH);runUap("main.urb");digitalWrite(LED_PIN,LOW);delay(200);break;case CMD_STOP:break;case CMD_OPEN_W:handleOpenW();break;case CMD_WRITE:handleWrite();break;case CMD_CLOSE:handleClose();break;case CMD_DEL:handleDel();break;case CMD_LIST_DIR:handleListDir();break;default:delay(10);break;}
}
