/*
 * Measure_Us.ino — Flash計測用 BASE + Us (ULTRASONIC)
 */
#include <Arduino.h>
#include <WebHID.h>
#include <SDmin.h>
  ...
  while(pc<codeSize){
    ...
    switch(opcode){
      ...
      case OP_ULTRASONIC:{
        ...
        uint32_t cnt=60000;while(!digitalRead(b[1])&&--cnt);
        uint32_t t0=SysTick->CNT;cnt=400000;while(digitalRead(b[1])&&--cnt);
