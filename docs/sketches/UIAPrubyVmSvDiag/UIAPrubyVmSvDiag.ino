...
#define SV_MAX_VARS 2  // DIAG  // RAM節約のため8個まで（2026-06-10 仕様変更）
...
static uint8_t _sv_sbuf[129];  // DIAG
static uint8_t _sv_lit[17];  // DIAG
  ...
  while (pc < codeSize) {
    ...
    switch (opcode) {
      ...
      case OP_VAR_LOAD: {
        uint8_t b[2]; if (sm_read_full(b,2)!=2) goto vm_err; pc+=2;
        sm_close_r();
        regs[b[1]&3] = sv_load_num(b[0] < SV_MAX_VARS ? b[0] : 0, 0);
      ...
      case OP_VAR_LOAD_IDX: {
        ...
        sm_close_r();
        regs[b[2]&3] = sv_load_num(b[0] < SV_MAX_VARS ? b[0] : 0, elem);
        if (!seekTo(filename, pc)) goto vm_err;
        break;
      }
      ...
      case OP_TO_S: {
        ...
        sm_close_r();
        memset(_sv_sbuf, 0, 129);
        for (uint8_t i = 0; i < slen; i++) _sv_sbuf[i] = (uint8_t)sbuf[i];
        sv_write_str(b[1] < SV_MAX_VARS ? b[1] : 0);
        if (!seekTo(filename, pc)) goto vm_err;
        break;
      }

      case OP_VAR_STR_SET: {  // var_idx, len, byte[len]
        uint8_t b[2]; if (sm_read_full(b,2)!=2) goto vm_err; pc+=2;
        uint8_t toRead = sv_read_lit(b[1]); if (toRead == 0xFF) goto vm_err;
        pc += b[1];
        sm_close_r();
        memset(_sv_sbuf, 0, 129);
        for (uint8_t i = 0; i < toRead; i++) _sv_sbuf[i] = _sv_lit[i];
        sv_write_str(b[0]);
        if (!seekTo(filename, pc)) goto vm_err;
        break;
      }

      case OP_VAR_STR_COPY: {  // dst, src
        uint8_t b[2]; if (sm_read_full(b,2)!=2) goto vm_err; pc+=2;
        sm_close_r();
        sv_read_str(b[1]); sv_write_str(b[0]);
        if (!seekTo(filename, pc)) goto vm_err;
        break;
      }

      case OP_VAR_STR_CAT: {  // dst, src（dst の末尾に src を連結、255B で切り捨て）
        uint8_t b[2]; if (sm_read_full(b,2)!=2) goto vm_err; pc+=2;
        sm_close_r();
        uint16_t d = sv_read_str(b[0]);
      ...
      case OP_VAR_STR_CAT_LIT: {  // dst, len, byte[len]
        uint8_t b[2]; if (sm_read_full(b,2)!=2) goto vm_err; pc+=2;
        uint8_t toRead = sv_read_lit(b[1]); if (toRead == 0xFF) goto vm_err;
        pc += b[1];
        sm_close_r();
