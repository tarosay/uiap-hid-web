/*
 * Measure_Base.ino — Flash計測用 BASE のみ
 * BASE オペコード: END / WAIT_MS / GPIO_* / JMP / JZ / JNZ / LOAD_BOOL / PRINT_STR / HALT
 */
  ...
  while (pc < codeSize) {
    ...
    switch (opcode) {
      ...
      case OP_GPIO_READ: {
        ...
        if (sm_read_full(b, 2) != 2) goto vm_err;
        pc += 2;
        regs[b[1] & 0x03] = digitalRead(b[0]) ? 1 : 0;
        break;
      }
      ...
      case OP_HALT: {
        uint8_t code;
        if (sm_read_full(&code, 1) != 1) goto vm_err;
        pc++;
