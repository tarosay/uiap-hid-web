/*
 * Measure_I2.ino — Flash計測用 BASE + I2 (I2C / Wiremin.h)
 */
#include <Arduino.h>
#include <WebHID.h>
#include <SDmin.h>
#include <Wiremin.h>  // I2C
#define LED_PIN 2
#define PIN_SS  6
...
#define OP_HALT      0x17
// ── I2 ──
#define OP_I2C_SLAVE_INIT  0x1B
#define OP_I2C_SLAVE_SET   0x1C
#define OP_I2C_SLAVE_GET   0x1D
...
static bool seekTo(const char *filename,uint16_t target_pc){sm_close_r();if(!sm_open_r(filename))return false;uint8_t hdr[8];if(sm_read_full(hdr,8)!=8)return false;uint16_t remain=target_pc;while(remain>0){uint8_t tmp[16];uint8_t n=remain>16?16:(uint8_t)remain;if(sm_read_full(tmp,n)!=n)return false;remain-=n;}return true;}
static bool runUap(const char *filename){
  ...
  while(pc<codeSize){
    ...
    switch(opcode){
      ...
      // ── I2 ──────────────────────────────────────────────────────
      case OP_I2C_SLAVE_INIT:{uint8_t b[1];if(sm_read_full(b,1)!=1)goto vm_err;pc++;Wiremin_slave_begin(b[0]);break;}
      case OP_I2C_SLAVE_SET:{uint8_t b[2];if(sm_read_full(b,2)!=2)goto vm_err;pc+=2;Wiremin_slave_set(b[0],(uint8_t)(regs[b[1]&3]&0xFF));break;}
      case OP_I2C_SLAVE_GET:{uint8_t b[2];if(sm_read_full(b,2)!=2)goto vm_err;pc+=2;regs[b[1]&3]=(int32_t)(uint8_t)Wiremin_slave_get(b[0]);break;}
      case OP_I2C_MASTER_INIT:{Wiremin_begin();break;}
      case OP_I2C_MASTER_GET:{uint8_t b[3];if(sm_read_full(b,3)!=3)goto vm_err;pc+=3;uint8_t val=0;Wiremin_read_reg(b[0],b[1],&val,1);regs[b[2]&3]=(int32_t)val;break;}
      case OP_I2C_MASTER_SET:{uint8_t b[3];if(sm_read_full(b,3)!=3)goto vm_err;pc+=3;uint8_t val=(uint8_t)(regs[b[2]&3]&0xFF);Wiremin_write_reg(b[0],b[1],&val,1);break;}
      // ────────────────────────────────────────────────────────────
