/*
 * Measure_Sv.ino — Flash計測用 BASE + Sv (SD変数)
 * Sv追加分: VAR_LOAD(0x25)/VAR_STORE(0x26)/VAR_LOAD_IDX(0x27)/VAR_STORE_IDX(0x28)
 *           + URB1 v2ヘッダ解析 + .urv ファイル管理
 */
#include <Arduino.h>
#include <WebHID.h>
#include <SDmin.h>
...
#define OP_HALT       0x17
#define OP_PRINT_REG  0x19  // to_s / Q16.8→文字列変換
// ── Sv: SD変数 ──
#define OP_VAR_LOAD      0x25  // uint8 var_idx, uint8 dst_reg
#define OP_VAR_STORE     0x26  // uint8 var_idx, uint8 src_reg
#define OP_VAR_LOAD_IDX  0x27  // uint8 var_idx, uint8 idx_reg, uint8 dst_reg
#define OP_VAR_STORE_IDX 0x28  // uint8 var_idx, uint8 idx_reg, uint8 src_reg
// ────────────────────────────────────────────────────────────────────────────
...
static uint16_t _sv_code_start = 8;  // コードセクション先頭のファイルオフセット
static char     _sv_prog[17];        // プログラム名 ("main" など)
...
// seekTo: _sv_code_start を使ってコードセクションの pc 位置に移動
...
static bool runUap(const char *filename){
    ...
    if(header[4]>=2){
        ...
        for(uint8_t i=0;i<_sv_count;i++){
            ...
            uint8_t elem_size=(_sv[i].type==1||_sv[i].type==3)?0 /* str=256 */:4;
            // 文字型は256バイト固定
            uint32_t sz;
    ...
    while(pc<codeSize){
        ...
        switch(opcode){
            ...
            case OP_VAR_LOAD_IDX:{
                ...
                // b[0]=var_idx, b[1]=idx_reg, b[2]=dst_reg
            ...
            case OP_VAR_STORE_IDX:{
                uint8_t b[3];if(sm_read_full(b,3)!=3)goto vm_err;pc+=3;
                // b[0]=var_idx, b[1]=idx_reg, b[2]=src_reg
                uint16_t elem=(uint16_t)(regs[b[1]&3]>>8);
                ...
                sm_close_r();
                // read-modify-write (スタックバッファ最大64バイト=16要素)
                uint8_t buf[64]={0};
            ...
            case OP_PRINT_REG:{uint8_t b[2];if(sm_read_full(b,2)!=2)goto vm_err;pc+=2;int32_t v=regs[b[1]&3];char buf[12];uint8_t len=0;if(v<0){buf[len++]='-';v=-v;}uint8_t ip=(uint8_t)(v>>8);uint8_t fp=(uint8_t)(((uint16_t)(v&0xFF)*100)>>8);if(ip>=100)buf[len++]='0'+ip/100;if(ip>=10)buf[len++]='0'+(ip/10)%10;buf[len++]='0'+ip%10;buf[len++]='.';buf[len++]='0'+fp/10;buf[len++]='0'+fp%10;consolePrint(buf,len,b[0]);break;}
