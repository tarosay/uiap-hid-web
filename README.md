...
**URB EE Lab — 新コンポーネント `Tm`（絶対周期）を追加**
...
- **Flash +580 B**（BASE 9,304 → 9,884 B 実測）。RAM は起点 8 個で 32 B
- 配布 bin 6 種には入っていない（ブザー版が残り 52 B しかなく、実測で溢れた）。
...
- **URB Lab（SD 版）にも同じ `Tm` を実装**した。命令番号も動作も EE 版と同一。
  Flash は **+384 B**（`UIAPrubyVm` 12,664 → `UIAPrubyVmTm` 13,048 B 実測）で、
  EE 版の +580 B より安い（SD 版の BASE が除算ヘルパを既にリンクしているため）
