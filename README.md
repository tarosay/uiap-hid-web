...
- **基板がつながらないとき（`⚙ 準備` の一番下）** — 「HID 接続チェックを開く」ボタンで
  [HID 接続チェック](https://tarosay.github.io/uiap-hid-web/hid-test.html?mode=boot) を別タブで開きます。
  「ファームウェアを書き込む」の一覧に基板が無い・書き込めないときに、ボタンを押しながら挿した基板をブラウザがどう見ているかを自分で確かめられます。
  ここから来る人のほとんどはファームウェアを書き込む前なので、`?mode=boot` を付けて、チェックの案内と判定を書き込みモード向けにしています
...
- **機器がつながらないときに、どこで止まっているかを自分で切り分ける**ための診断ページ。スケッチ不要・1 ファイル完結・外部ライブラリなし。**機器には何も書き込まない**
- **表に出すのは「チェックする」ボタン 1 つと、3 行の結果だけ。**ボタン 1 回で「ブラウザ → 機器を選べる → 機器を開ける」を続けて行い、
  各行に ✔（問題なし）／！（確かめること）／✖（つまずき）を出す。**全部 ✔ なら「OK　この機器はブラウザから使えます」と、何の機器だったかだけを出す**
- **つまずいたら、その行に短い理由を出し、下の「くわしく」の該当する段を自動で開いて赤枠で示し、そこまでスクロールする。**各行の「くわしく」からも開ける
- ブラウザが対応していないときは、開いた瞬間に ✖ とつまずいた場所を出し、ボタンを押せなくする
- 説明は全部「くわしく」の折りたたみの中。判定の色は 緑＝問題なし／青＝いまの状態の説明／黄＝確かめること／赤＝つまずき／灰＝このページでは判定しない
  - ① ブラウザ: `navigator.hid` の有無 / `window.isSecureContext` / `location.protocol` / `userAgent`。
    使えないときは、PC の Chrome・Edge で開く／`https://` で開き直す、を案内（Android は別の案内）
  - ② 機器を選べる: `requestDevice({ filters: [] })` を**ボタン 1 回につき 1 回**、ほかの await より先に呼ぶ（クリック直後でないと出せないため）。
    絞り込まないので、選ぶ画面がそもそも出るかが分かる。`SecurityError`・`NotAllowedError`・選ばれなかったとき、それぞれに意味と次にやることを出す。
    同じ段に「このサイトで許可済みの機器」（`getDevices()`。開いた時点・USB の抜き挿し・チェックのあとに取り直す）
  - ③ 機器を開ける: 選んだ機器を `open()` してすぐ `close()`。開けなければ、ほかのアプリ・ほかのブラウザが使っていないか、挿し直しを案内する。
    このページが開いたものだけを閉じる
  - 「**UIAPduino の 2 通りの挿し方**」（見出しの下の「挿し方」リンクからも開く）
    - ふだん（ボタンを押さずに挿す）… `1209:D004`「UIAPduino WebHID」。arduino_core_ch32 で作ったプログラムが名乗る番号
    - 書き込みモード（ボタンを押しながら挿して、すぐ離す）… `1209:B803`、OS の画面では「32V003」。UIAP が付けた番号。終わらせる処理をしない限り続く
- **入口で案内と判定が変わる。**
  - `?mode=boot`（URB Block Lab の「⚙ 準備」から）… 冒頭は「ファームウェアを書き込む前の確認です。ボタンを押しながら挿して、すぐ離してください」。
    32V003（`1209:B803`）なら ✔「OK　ファームウェアを書き込める状態です」、ふだんの状態（`1209:D004`）や別の機器なら ！で挿し直しを案内する。
    一覧に何も選ばれなかったときも、ボタンを押しながら挿し直すよう案内する
  - 印なし（トップページ・直接のリンク）… 冒頭に 2 通りの挿し方を両方出し、どちらの状態でも開ければ OK
- **名前で判定するのは、知っている番号（`KNOWN` の表。いまは `1209:D004`・`1209:B803`）だけ。**
  それ以外の機器は「異常」と言わず、灰色で「このページが知っている番号ではありません」と生の値を出す。番号は `KNOWN` に 1 行足せば増やせる
- 機器ごとに `productName`（空文字なら「(空文字)」）/ `vendorId` / `productId`（10 進と 16 進）/ `usagePage`/`usage`（collections の全数）を表示。
  失敗時は `error.name` と `error.message` の**両方**を出す
- 巻末に折りたたみで「**しくみ**」（つながるまでの 3 段階＝選べる・開ける・やり取りできる／許可はサイトごと・機器ごと／
  選ぶ画面はクリック直後だけ／UIAPduino は 2 つの機器に見える／番号の読み方）と「**結果の早見表**」
...
docs/                           ← GitHub Pages のルート
  ...
  hid-test.html                 ← HID 接続チェック（ボタン 1 つの簡易チェック。requestDevice / open / getDevices と判定）
...
- **生の値を並べるだけでは、読む人が知っている前提になるので、結果を判定するページに作り直しました。**
  各段に判定（緑・青・黄・赤・灰）と次にやることを出し、冒頭に使い方と UIAPduino の 2 通りの挿し方
  （ふだん `1209:D004`／書き込みモード `1209:B803`・OS の画面では「32V003」）を載せました。
  書き込みモードは異常ではなく、ファームウェアを書き込むときの正しい状態として判定します
- **「④ 開けるか試す」を足しました。**「選べる」までしか見ていなかったので、開くときに初めて出るエラー
  （ほかのアプリが使っている等）が見えませんでした。開いてすぐ閉じるだけで、何も書き込みません
- **知らない番号の機器は「異常」と言いません。**このページが一人歩きして、UIAPduino 以外の機器を調べに来る人もいるためです。
  `1209:D004` は arduino_core_ch32、`1209:B803` は UIAP が付けた番号で、同じ CH32V003 でも別の基板・別のコアでは番号が違うことも書きました
- 巻末に「しくみ」と「結果の早見表」を折りたたみで置きました。表題は「WebHID 列挙テスト」から「HID 接続チェック」に（ファイル名は `hid-test.html` のまま）
- **文章が多すぎて誰も使わない、という指摘を受けて、表を「簡易チェック」だけにしました。**
  「チェックする」を 1 回押すと、ブラウザ → 機器を選べる → 機器を開ける を続けて行い、3 行の ✔／！／✖ と「OK」だけを出します。
  つまずいたときだけ、その段のくわしい説明を自動で開きます。説明はすべて「くわしく」の折りたたみの中に移しました
- **入口で案内を変えました。**URB Block Lab の「⚙ 準備」から来る人のほとんどは、これからファームウェアを書き込む人です。
  それなのに冒頭で「ボタンを押さずに挿してください」と、逆の案内をしていました。
  「⚙ 準備」のボタンは `hid-test.html?mode=boot` を開き、ページはこの印を見て、書き込みモード（32V003）を正解として案内・判定します。
  印の無いとき（トップページや Discord に貼ったリンク）は、2 通りの挿し方を両方出します。
  「⚙ 準備」の節の説明も、「基板に書き込む」ではなく「ファームウェアを書き込む」でつまずいた人向けに書き直しました
