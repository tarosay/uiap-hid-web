...
| [URB Block Lab](https://tarosay.github.io/uiap-hid-web/uiapruby-block.html) | 同梱ファーム 1 本（`⚙ 準備` から書き込み） | 🚧 作成中（本基板の到着待ち） |
...
> ⚠ **このページは作成中です。**同梱ファームは CAT24M01WI **0x50/0x51** 向けで、
> いまは**仮組の基板**で動かしています。**本基板での確認はこれからです。**
> ページには「作成中」の透かしを出しています。
...
> ⚠ 実機で確認したのは **0x52/0x53 版**です（そのとき使ったテスト基板がその配線でした）。
> 配っている **0x50/0x51 版は、まだ実機で動かしていません**。
