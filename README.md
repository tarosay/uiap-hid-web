...
> ⚠ **このページは作成中です。**同梱しているファームがテスト基板
> （CAT24M01WI **0x52/0x53**）向けのままなので、0x50/0x51 の基板には書き込めません。
...
- **URB Block Lab に「作成中」の透かし。** 同梱ファームがテスト基板（0x52/0x53）向けの
  ままなので、配れる状態ではないことを画面に出す。0x50/0x51 版に戻したら消す
