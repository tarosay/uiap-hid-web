...
- **`⚙ 準備`（大人の方へ）** — ファームウェアの書き込みはここに入れてあります。基板 1 台につき一度だけ。
  開くと必ず先頭（手順）から表示します。「閉じる」・Esc のほか、外側をクリックしても閉じます
  （書き込み・調べている最中は、外をクリックしても閉じません）
...
- **基板がつながらないとき（`⚙ 準備` の一番下）** — 「HID 接続チェックを開く」ボタンで
  [HID 接続チェック](https://tarosay.github.io/uiap-hid-web/hid-test.html) を別タブで開きます。
  選ぶ画面が出ない・つながらないときに、ブラウザが基板をどう見ているかを自分で確かめられます
...
docs/                           ← GitHub Pages のルート
  ...
  hid-test.html                 ← HID 接続チェック（つながらないときの切り分け。getDevices / requestDevice）
...
- 基板がつながらないときに、**どこで止まっているかを切り分ける**ための診断ページです。スケッチは要りません
- 講習で URB Block Lab を使ってもらったとき、ファームウェアの書き込みは全員できたのに、
  「基板に書き込む」で**デバイスを選ぶダイアログが出ない** PC がありました。
  DevTools にコードを貼ってもらう手順書では、`allow pasting` の壁や、1 回の貼り付けで
  `requestDevice` を 2 回呼ぶとユーザー操作の期限が切れて 2 回目が空になる事故がありました。
  URL を開いてボタンを 1 回押すだけで同じことが分かるように、ページにしました
- ページを開いた時点で `getDevices()` を自動で実行し、許可済みの機器を出します（0 件ならそう出ます）
- ボタンを押すと `requestDevice({ filters: [] })` を 1 回だけ実行します。絞り込まないので、ダイアログがそもそも出るかが分かります
- 機器の `vendorId` / `productId`（10 進と 16 進）と `usagePage`/`usage`（正常な UIAPduino は `0xFF00/0x0001`）、
  失敗したときの `error.name` と `error.message` を画面にそのまま出します
- トップページの「デバッグ / ユーティリティ」に 3 枚目のカードとして載せました
- URB Block Lab の「⚙ 準備」の一番下に「基板がつながらないとき」の節と「HID 接続チェックを開く」ボタンを足し、
  このページへ案内しました（ボタンの見た目は「基板のファームウェアを調べる」とそろえています）

**URB Block Lab — 「⚙ 準備」のダイアログを直した**

- **開くと先頭が隠れていた。** 開いた瞬間にブラウザが最初のボタン「ファームウェアを書き込む」へフォーカスを移し、
  そこまでスクロールするので、「はじめの準備（大人の方へ）」と手順が上に隠れていました。開いた直後に先頭へ戻します
- **外側をクリックしても閉じるようにした。** これまでは「閉じる」か Esc でしか閉じませんでした。
  ただし、ファームウェアの書き込み中・調べている最中は閉じません（進み具合が見えなくなり、終わったと思って USB を抜かれるため）。
  ダイアログの中で押して外で離したとき（文字を選んでいた等）も閉じません
