...
| EP0 | WebHID RX: Web→デバイス（Feature Report、**スケッチ依存**。arduino_core_ch32 v1.1.5 以降は最大 **32 バイト**） |

### EP0 Feature Report の注意点（現行）

arduino_core_ch32 v1.1.5 で Feature Report サイズは **16 → 32 バイト** に変更された。  
そのため、このリポジトリのブラウザ側実装はデバイスの HID ディスクリプタから Feature Report サイズを自動検出して扱う。  
旧 16 バイト前提のスケッチや説明が混在する可能性があるため、実際のサイズは各スケッチの `recv` バッファサイズとディスクリプタを確認する。
...
|------|----|
| スケッチ | `WebHIDTest.ino` |
| 動作 | Feature Report を送信 → エコーバック受信 + 1秒カウンター（`WebHIDTest.ino` は 16 バイト受信） |
| WebHID filter | `{ vendorId: 0x1209, productId: 0xD004, usagePage: 0xFF00, usage: 0x01 }` |
| 送信 | `device.sendFeatureReport(0, uint8Array)` |
...
| テキスト送信 | textarea + 送信ボタン。Feature Report サイズに応じて複数回送る |
...
### スケッチ動作仕様
- WebHID.recv() で Feature Report サイズに応じて受信
